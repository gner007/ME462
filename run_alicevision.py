...
def Run_07_DepthMap(baseDir,binDir,numImages,groupSize,srcImageDir):
	...
	#cmdLine = cmdLine + " --ini \"" + srcIni + "\""
	cmdLine = cmdLine + " --imagesFolder \"" + baseDir + "/05_PrepareDenseScene" + "\""
	srcSfm = baseDir + "/04_StructureFromMotion/bundle.sfm"
	cmdLine = cmdLine + " --input \"" + srcSfm + "\""
	...
	for groupIter in range(floor(numGroups)+1):
...
def Run_09_Meshing(baseDir,binDir):
	...
	cmdLine = cmdLine + " --output \"" + dstDir + "/mesh.abc\""
...
def Run_11_Texturing(baseDir,binDir):
	...
	srcRecon = baseDir + "/09_Meshing/mesh.abc"
	srcIni = baseDir + "/05_PrepareDenseScene/mvs.ini"
	...
	cmdLine = cmdLine + " --input \"" + srcRecon + "\""
